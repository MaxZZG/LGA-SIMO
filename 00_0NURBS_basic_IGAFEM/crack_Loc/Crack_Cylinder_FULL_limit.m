...
path (path, '~/code/mosek/6/tools/platform/osx64x86/bin')
path (path, '~/code/mosek/6/toolbox/r2009b')
...
for i = 1:size(CP_ovl,2)
    Bc(2*(i-1)+1,2*CP_ovl(:,i)-1) = [1 -1];
    Bc(2*(i-1)+2,2*CP_ovl(:,i)) = [1 -1];
end
...
%Nel
result
exact = 2/sqrt(3)*log(R_out/(r_in-a))
% result*2/pi
% break
figure
...
while count <= count_tri
    ...
    if max(ll) >= 1.5*R_out*pi/Numx/(noGPs-1)
        tri(i,:)=[];
        i=i-1;
    end
    end
