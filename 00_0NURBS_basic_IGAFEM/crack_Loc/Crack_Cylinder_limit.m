...
upperNodes  = 1 : noPtsX;
lowerNodes  = noPtsX*(noPtsY-1)+1 : noCtrPts;
% topNodes    = sort(topNodes,'descend');

leftNodes   = noPtsX : noPtsX : noCtrPts;
rightNodes  = 1: noPtsX : noCtrPts;
% build boundary mesh

upperPoints    = controlPts(upperNodes,:);
rightPoints    = controlPts(rightNodes,:);
lowerPoints    = controlPts(lowerNodes,:);
leftPoints     = controlPts(leftNodes,:);

% Loop over elements along top edge 

[elRangeU,elConnU] = buildConnectivity(p,uKnot,Numx);
lowerEdgeMesh  = lowerNodes(elConnU);
upperEdgeMesh  = upperNodes(elConnU);
...
for e=1:Numx   
    ...
    sctry = lowerEdgeMesh(e,:);
    pts   = controlPts(lowerEdgeMesh(e,:),:);
...
bcdof = [rightNodes*2,leftNodes*2-1];
Wex = f'; % External Work
clear f
