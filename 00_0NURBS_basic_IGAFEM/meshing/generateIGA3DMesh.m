...
for i=1:noElemsU
    uKnotElem(i,:) = uKnot(i:i+p+1+2);    
end
for i=1:noElemsV
    vKnotElem(i,:) = vKnot(i:i+q+1+2);    
end
for i=1:noElemsW
    wKnotElem(i,:) = wKnot(i:i+r+1+2);    
end

uvKnotElem = cell(size(element,1),1);
weightElem = cell(size(element,1),1);
sp = 1;
for i = 1:noElemsW
    for j = 1:noElemsV
        for k = 1:noElemsU
        uvKnotElem{sp} = [uKnotElem(k,:);vKnotElem(j,:);wKnotElem(i,:)];
        sctr = element(sp,:);
        weightElem{sp} = weights(sctr);
        sp = sp + 1;
        end
    end
end
...
for i=1:noElemsW
    for j=1:noElemsV
        for k=1:noElemsU
            elemUVW = [elemUVW; elRangeU(k,:), elRangeV(j,:),elRangeW(i,:)];
        end
    end
end
