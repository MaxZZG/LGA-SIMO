...
function [ders]=dersbasisfuns(i,pl,order_deriv,u,u_knotl)

%     --------------variable declarations--------------------------------
% Input: i,pl,order_deriv   knot span, degree of curve, number of
%                           derivatives
%        u , u_knotl  parameter value, vector of knots
% Out:   ders(1,:)  vector with values of B-spline shape functions
%        ders(2,:)  vector with values of 1st derivative B-Spline basis
%      ...
%        ders(order_deriv, :) vector with values of order_deriv-th
%                             derivative of B-spline basis

left=zeros(1,pl+1);
right=zeros(1,pl+1);
ndu=zeros(pl+1,pl+1);
a=zeros(order_deriv+1,pl+1);
ders=zeros(order_deriv+1,pl+1);
